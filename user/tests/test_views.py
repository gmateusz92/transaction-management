...
class UserTests(APITestCase):
    ...
    def test_register_user(self):
        ...
        self.assertEqual(
            User.objects.get(email="john2@example.com").email, "john2@example.com"
        )
    ...
    def test_reset_password_mismatched_passwords(self):
        url = reverse("reset_password", args=["testtoken"])  # Use the correct token
        data = {"password": "newpassword123", "confirmPassword": "newpassword456"}
        response = self.client.post(url, data, format="json")
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
